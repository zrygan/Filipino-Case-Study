...
\begin{enumerate}
      \item \(\left(\forall s\in \mathcal{S},s \matches \texttt{es}\mathbb{S}^*\right) \rightarrow \left(\mathcal{F}(s) \matches \texttt{es}\Sigma_\text{lower}^*\right) \)
      \item \(\left(\forall s\in \mathcal{E},s \matches \texttt{s}\mathbb{E}^*\right) \rightarrow  \left(\mathcal{F}(s) \matches \texttt{is} \Sigma_\text{lower}^* \right)\)
\end{enumerate}
...
In particular:
\begin{itemize}
      \item \(s \matches (\texttt{conf})\mathbb{S}^* \rightarrow \mathcal{F}(s) \matches (\texttt{kump})\Sigma^*\)
      \item \(s \matches (\texttt{conv})\mathbb{S}^* \rightarrow \mathcal{F}(s) \matches (\texttt{kumb})\Sigma^*\)
\end{itemize}
...
If
\[
    S = \alpha + r + EP \land \alpha \matches (\mathbb{L}_\text{upper}\mathbb{L}_\text{lower}^*\mathbb{V}) \Longrightarrow EP = \texttt{"raw"}. 
\]
Otherwise,
\[
      EP = \text{"daw"}
...
\(S = \omega + p + (\text{"rin"}\mid\text{"din"})\) and \(\omega\) ends in a vowel
